"""
Cryptographic utilities for BLE symmetric key authentication (Client).
...
def derive_device_key(master_key: bytes, device_id: bytes) -> bytes:
    ...
    Note: This is used for POC key derivation utility only.
    In production, the client would receive the pre-derived key during enrollment.
